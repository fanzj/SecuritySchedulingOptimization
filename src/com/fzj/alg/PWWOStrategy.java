 ...
 * @description
 */
public class PWWOStrategy extends AStrategy {
	...
	public PWWOStrategy(int f_aI4_size, int f_aI4_max_nfe, int f_aI4_max_iter, SSModel f_aTC_ssm,
	...
	protected void init() {
		...
		for (int t_aI4_i = 0; t_aI4_i < m_aI4_d; t_aI4_i++) {
			m_rI8_range[t_aI4_i] = m_aI4_upper - 1;
		}
		...
		m_str_alg_name = NameSpace.s_str_pwwo;
	...
	private void initV(){
		for(int t_aI4_k=0;t_aI4_k<m_aI4_size;t_aI4_k++){
			WaveSolution t_aTC_wave = (WaveSolution) m_aTC_population[t_aI4_k];
			int[] t_aI4_x = t_aTC_wave.getM_rI4_x();
			t_aTC_wave.setM_rI4_v(t_aI4_x);
		}
	}
	...
	private void propagate(WaveSolution f_aTC_wave) {
		int[] t_rI4_best_x = m_aTC_best.getM_rI4_x();

		double t_aI8_w = f_aTC_wave.getM_aI8_w();
		int[] t_rI4_x = f_aTC_wave.getM_rI4_x();
		int[] t_rI4_v = f_aTC_wave.getM_rI4_v();
		int[] t_rI4_new_v = new int[m_aI4_d];
		for (int t_aI4_j = 0; t_aI4_j < m_aI4_d; t_aI4_j++) {
			double t_aI8_r = MathUtils.getDoubleAToB(-1, 1);
			// double t_aI8_y =
			// t_aI4_xd+Math.ceil(t_aI8_r*t_aI8_w*m_rI8_range[t_aI4_j]);
			t_rI4_new_v[t_aI4_j] = new Double(Math
					.ceil(t_aI8_w * t_rI4_v[t_aI4_j] + m_aI8_c1 * t_aI8_r * (t_rI4_best_x[t_aI4_j] - t_rI4_x[t_aI4_j])))
							.intValue();
			if(t_rI4_new_v[t_aI4_j]>m_aI4_vmax){
				t_rI4_new_v[t_aI4_j]=m_aI4_vmax;
			}
			int t_aI4_temp = t_rI4_x[t_aI4_j]+t_rI4_new_v[t_aI4_j];
			if (t_aI4_temp < m_aI4_low)
				t_aI4_temp = m_aI4_low;
			if (t_aI4_temp > m_aI4_upper)
				t_aI4_temp = m_aI4_upper;
			t_rI4_x[t_aI4_j] = t_aI4_temp;
		}
		f_aTC_wave.setM_rI4_x(t_rI4_x);
		f_aTC_wave.setM_rI4_v(t_rI4_new_v);
		modify(f_aTC_wave);
		evaluate(f_aTC_wave);
	...
	private void refract(WaveSolution f_aTC_wave) {
		...
		for (int t_aI4_j = 0; t_aI4_j < m_aI4_d; t_aI4_j++) {
			...
			int t_aI4_temp = new Double(Math.ceil(t_aI8_r)).intValue();
			if (t_aI4_temp < m_aI4_low || t_aI4_temp > m_aI4_upper)
				t_aI4_temp = m_aI4_low + m_aTC_random.nextInt(m_aI4_upper);
	...
	private void breaking(WaveSolution f_aTC_wave) {
		...
		for (int t_aI4_i = 0; t_aI4_i < t_aI4_k; t_aI4_i++) {
			...
			int t_aI4_temp = new Double(Math.ceil(t_aI8_y)).intValue();
			if (t_aI4_temp < m_aI4_low || t_aI4_temp > m_aI4_upper)
				t_aI4_temp = m_aI4_low + m_aTC_random.nextInt(m_aI4_upper);
	...
	protected void evolution() {
		...
		System.out.println("m_aI4_cur_nfe = " + m_aI4_cur_nfe);
		System.out.println("m_aI4_cur_iter = " + m_aI4_cur_iter);
	}
	...
	protected ASolution solve(int f_aI4_p) {
		...
		{
			init();
			initPopulation(NameSpace.s_str_pwwo);
			initPopulation2();
			initV();
			updateBestAndWorst();
		}
	...
	public static void main(String[] args) {
		...
		AStrategy t_aTC_strategy = new PWWOStrategy(10, 1000, 500, t_aTC_ssm, NameSpace.s_str_data_01);
